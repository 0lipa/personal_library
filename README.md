...
- 데이터는 **각 기기의 브라우저 저장소(localStorage)** 에 저장됩니다. 따라서 이 정적 배포판만으로는 다른 기기나 학생 화면에 자동 공유되지 않습니다.
- `선생님 모드`는 정적 데모/개인 기기용입니다. 비밀번호나 계정 보호가 없으므로 실제 공개 서비스의 권한 기능으로 사용하면 안 됩니다.
